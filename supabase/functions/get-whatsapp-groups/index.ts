...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`📦 Active instances for group lookup: ${instances.length}`);
      ...
      try {
        ...
        while (hasMore) {
          ...
          if (!response.ok) {
            ...
            console.error(
              `❌ Z-API error for instance ${instance.instance_name || instance.zapi_instance_id}: ${response.status} - ${errorText}`,
            );
            break;
          }
          ...
          console.log(
            `📄 Instance ${instance.instance_name || instance.zapi_instance_id} | Page ${page}: ${groups.length} groups`,
          );
        ...
        console.error(
          `❌ Failed to fetch groups for instance ${instance.instance_name || instance.zapi_instance_id}:`,
          instanceError,
        );
      }
    }
