...
function isTelegramFlow(flow: any): boolean {
  return String(flow?.category || "contacts").toLowerCase() === "telegram";
}
  ...
  try {
    ...
    if (!fromMe || isButtonResponse) {
      ...
      for (const flow of flows || []) {
        if (!isTelegramFlow(flow)) {
           console.log(`[webhook-telegram-v2] skipping non-telegram flow "${flow.name}"`, { category: flow.category });
           continue;
        }
