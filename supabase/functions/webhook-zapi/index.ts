...
async function resolveUazapiPendingButtonReplyFromHistory(params: {
  ...
  webhook: any;
  pendingState: PendingButtonState;
}): Promise<{ matchedText: string; matchedButtonId: string } | null> {
  const apiUrl = String(params.apiUrl || "").replace(/\/+$/, "");
  const apiToken = String(params.apiToken || "").trim();
  const buttons = Array.isArray(params.pendingState?.buttons)
    ? params.pendingState.buttons
    : [];

  if (!apiUrl || !apiToken || buttons.length === 0) return null;

  const cleanId = String(params.phone || "").replace(/-group$/i, "").replace(
    /\D/g,
    "",
  );
  const chatCandidates = params.phone.includes("-group")
    ? [`${cleanId}@g.us`, cleanId]
    : [`${cleanId}@s.whatsapp.net`, `${cleanId}@c.us`, cleanId];

  const technicalReferences = new Set<string>();
  const pushTechnicalReference = (value: unknown) => {
    const raw = String(value || "").trim();
    if (!raw) return;

    technicalReferences.add(raw.toUpperCase());
    const technicalMatch = raw.match(/^\d{10,}:([A-Z0-9]{10,})$/i);
    if (technicalMatch?.[1]) technicalReferences.add(technicalMatch[1].toUpperCase());
  };

  [
    params.rawMessage,
    params.webhook?.messageId,
    params.webhook?.text?.selectedButtonId,
    params.webhook?.text?.selectedRowId,
    params.webhook?.buttonReply?.selectedButtonId,
    params.webhook?.buttonReply?.selectedRowId,
  ].forEach(pushTechnicalReference);

  const matchPendingButton = (values: string[]) => {
    for (const value of values) {
      const normalizedValue = normalizeForMatch(value);
      if (!normalizedValue) continue;

      for (const button of buttons) {
        const menuIndex = button.menuIndex ?? button.index + 1;
        const normalizedOptions = [
          button.text,
          String(menuIndex),
          `${menuIndex}. ${button.text}`,
          ...button.handleAliases,
        ].map((option) => normalizeForMatch(option)).filter(Boolean);
  ...
  let fallbackMatch: { matchedText: string; matchedButtonId: string } | null = null;
  ...
  for (const chatid of chatCandidates) {
    try {
      ...
      if (!resp.ok) {
        console.warn(`⚠️ UAZAPI /chat/messages falhou para ${chatid}: ${resp.status}`);
        continue;
      }
      ...
      for (const item of messages) {
        const replyValues = Array.from(
          new Set([
            ...extractButtonReplyCandidates(item),
            ...extractButtonReplyCandidates({ message: item }),
            ...extractButtonReplyCandidates(item?.message || {}),
            ...extractButtonReplyCandidates({ message: item?.message || {} }),
            extractMessageText(item),
            extractMessageText({ message: item }),
            extractMessageText(item?.message || {}),
            extractMessageText({ message: item?.message || {} }),
          ].filter((value): value is string =>
            typeof value === "string" && value.trim().length > 0
          )),
        );

        const matchedButton = matchPendingButton(replyValues);
        ...
        const pushMessageId = (value: unknown) => {
          ...
          if (!rawValue) return;
          messageIds.add(rawValue.toUpperCase());
          const technicalMatch = rawValue.match(/^\d{10,}:([A-Z0-9]{10,})$/i);
          if (technicalMatch?.[1]) messageIds.add(technicalMatch[1].toUpperCase());
        };
        ...
        [
          ...
          item?.message?.key?.id,
          item?.message?.stanzaId,
          item?.buttonResponseMessage?.stanzaId,
          item?.message?.buttonResponseMessage?.stanzaId,
          item?.templateButtonReplyMessage?.stanzaId,
          item?.message?.templateButtonReplyMessage?.stanzaId,
          item?.contextInfo?.stanzaId,
          item?.message?.contextInfo?.stanzaId,
        ].forEach(pushMessageId);

        const hasTechnicalMatch = Array.from(messageIds).some((candidate) =>
          Array.from(technicalReferences).some((reference) =>
            candidate === reference || candidate.endsWith(reference) ||
            reference.endsWith(candidate)
          )
        );

        const resolved = {
          matchedText: matchedButton.text,
          matchedButtonId: `button-${matchedButton.index}`,
        };

        if (hasTechnicalMatch) {
          console.log(
            "🧭 UAZAPI history resolved technical button reply",
            { chatid, technicalReferences: Array.from(technicalReferences), resolved },
          );
          return resolved;
        }

        if (!fallbackMatch) fallbackMatch = resolved;
      }
    } catch (error) {
      console.warn("⚠️ Erro ao consultar histórico recente da UAZAPI:", error);
    }
  }

  if (fallbackMatch) {
    console.log("🧭 UAZAPI history fallback matched pending button", fallbackMatch);
  }
