...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!paymentStatus && !orderStatus) throw new Error("paymentStatus ou orderStatus é obrigatório");

    const creds = await resolveCreds(req, instanceDbId);

    const url = `https://api.z-api.io/instances/${creds.instanceId}/token/${creds.token}/order-payment-update`;
    ...
    if (!resp.ok || data?.error) {
      console.error("❌ order-payment-update failed", resp.status, data);
      return new Response(
        JSON.stringify({ error: data?.error || data?.message || "Falha ao atualizar pagamento do pedido", details: data }),
        { status: resp.status || 502, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }

    console.log("✅ order-payment-update ok", data);
    return new Response(JSON.stringify({ success: true, data }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error("❌ send-order-payment-update error:", message);
