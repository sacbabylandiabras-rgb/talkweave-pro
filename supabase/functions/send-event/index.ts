...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!startTime) throw new Error("startTime é obrigatório");

    const creds = await resolveCreds(req, instanceDbId);

    const url = `https://api.z-api.io/instances/${creds.instanceId}/token/${creds.token}/send-event`;
    ...
    if (!resp.ok || data?.error) {
      console.error("❌ send-event failed", resp.status, data);
      return new Response(
        JSON.stringify({ error: data?.error || data?.message || "Falha ao enviar evento", details: data }),
        { status: resp.status || 502, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }

    console.log("✅ send-event ok", data);
    return new Response(JSON.stringify({ success: true, data }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error("❌ send-event error:", message);
