...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!messageIdToEdit) throw new Error("messageIdToEdit é obrigatório");
    if (!title && !cancelEvent) throw new Error("title é obrigatório (a menos que cancelEvent seja true)");
    if (!startTime && !cancelEvent) throw new Error("startTime é obrigatório (a menos que cancelEvent seja true)");

    const creds = await resolveCreds(req, instanceDbId);

    const url = `https://api.z-api.io/instances/${creds.instanceId}/token/${creds.token}/send-edit-event`;
    ...
    if (typeof isAllDay === "boolean") payload.isAllDay = isAllDay;
    if (eventUrl) payload.url = eventUrl;
    if (typeof cancelEvent === "boolean") payload.cancelEvent = cancelEvent;
    ...
    if (!resp.ok || data?.error) {
      console.error("❌ send-edit-event failed", resp.status, data);
      return new Response(
        JSON.stringify({ error: data?.error || data?.message || "Falha ao editar evento", details: data }),
        { status: resp.status || 502, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }

    console.log("✅ send-edit-event ok", data);
    return new Response(JSON.stringify({ success: true, data }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error("❌ send-edit-event error:", message);
