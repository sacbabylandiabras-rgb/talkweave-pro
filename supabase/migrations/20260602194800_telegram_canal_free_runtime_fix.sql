-- Ensure Canal Free tables exist in projects where the first migration was not applied yet.
...
DROP POLICY IF EXISTS "users select own free channels" ON public.telegram_free_channels;
DROP POLICY IF EXISTS "users insert own free channels" ON public.telegram_free_channels;
DROP POLICY IF EXISTS "users update own free channels" ON public.telegram_free_channels;
DROP POLICY IF EXISTS "users delete own free channels" ON public.telegram_free_channels;
...
DROP TRIGGER IF EXISTS trg_telegram_free_channels_updated ON public.telegram_free_channels;
...
DROP POLICY IF EXISTS "users select own join requests" ON public.telegram_free_join_requests;
