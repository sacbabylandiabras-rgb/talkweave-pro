...
const isConversationBoundInstanceLog = (
  log: Pick<MessageLog, "instance_id" | "message_received" | "keyword_matched">,
) => {
...
// FIX: inclui 'sent' e 'read' além de 'delivered' — antes só 'delivered' era visível
const isCampaignMessageVisible = (send: CampaignSendMessage) =>
  send.status === "delivered" || send.status === "sent" || send.status === "read";

const getCampaignSendTimestamp = (send: Pick<CampaignSendMessage, "sent_at" | "created_at">) =>
  send.sent_at || send.created_at;

const isHistoryPlaceholderText = (content?: string | null) =>
  /^💬\s*Conversa com\s+/i.test(String(content || "").trim());
...
const isHistoryPlaceholderMessage = (message: Pick<UnifiedMessage, "source" | "keyword_matched" | "content">) => {
  ...
  const isDeleted =
    content.includes("esta mensagem foi apagada") ||
...
const isInternalFlowStateKeyword = (keyword?: string | null) => {
  ...
  return (
    value.startsWith("__flow_button__:") ||
...
const isTechnicalMessageReference = (content?: string | null) =>
  /^\d{10,}:[A-Z0-9]{10,}$/i.test(String(content || "").trim());

const normalizeSentMessageForComparison = (content?: string | null) =>
  String(content || "")
    .replace(/\[media:[^\]]+\]\s*/gi, "")
    .replace(/\n\n\[Botões:\s*[\s\S]*$/i, "")
    .replace(/\s+/g, " ")
...
const isRedundantManualFlowEcho = (
  ...
  return logs.some((candidate) => {
    ...
    const samePayload =
      flowContent === manualContent ||
      flowContent.startsWith(`${manualContent} `) ||
      flowContent.startsWith(manualContent);
...
const parseSenderFromContent = (
  raw: string,
): { name: string | null; phone: string | null; photo: string | null; rest: string } => {
...
const isGroupMembershipLog = (log: Pick<MessageLog, "message_received" | "keyword_matched">) =>
  (log.keyword_matched === "__group_join__" || log.keyword_matched === "__group_leave__") &&
  isGroupPhone(String(log.message_received || ""));
...
const resolveGroupMembershipContent = (log: Pick<MessageLog, "phone" | "response_sent" | "keyword_matched">) => {
  ...
  const joinedPhone = phone.toLowerCase().includes("@lid") ? phone.toLowerCase() : phone.replace(/\D/g, "");
...
const getLatestSuccessfulCampaignSends = (sends: CampaignSendMessage[]) => {
  ...
  sends.filter(isCampaignMessageVisible).forEach((send) => {
    const current = latestByPhone.get(send.phone);
    if (
      !current ||
      new Date(getCampaignSendTimestamp(send)).getTime() >= new Date(getCampaignSendTimestamp(current)).getTime()
    ) {
...
const extractResolvedGroupName = (payload: any): string | null => {
  ...
  if (typeof payload?.data?.name === "string" && isUsableGroupDisplayName(payload.data.name))
    return payload.data.name.trim();
  return null;
};
...
export const useMessageLogs = (
  ...
  const fetchSavedContacts = useCallback(async () => {
    try {
      ...
      data.forEach((c) => {
        map.set(c.phone, c);
        if (isGroupPhone(c.phone)) map.set(normalizeConversationPhone(c.phone), c);
  ...
  const fetchMessageLogs = useCallback(async (currentUserId: string | null) => {
    ...
    if (cachedDeleted) JSON.parse(cachedDeleted).forEach((p: string) => localDeleted.add(p));
    ...
    allData = allData.map((m) => {
      ...
      if (isLikelyTechnicalIdentifier(workingPhone)) {
        ...
        if (lidEvidence.has(suspectLid)) workingPhone = suspectLid;
      }
  ...
  const fetchLidMap = useCallback(async () => {
    ...
    if (data) {
      ...
      data.forEach((r: any) => {
        if (r.message_received && r.phone) map.set(r.message_received, r.phone);
      });
  ...
  const fetchCampaignSends = useCallback(async (currentUserId: string | null) => {
    ...
    while (hasMore && allData.length < maxRecords) {
      const { data, error } = await supabase
        ...
        .eq("user_id", currentUserId)
        // FIX: inclui 'sent' e 'read' além de 'delivered'
        // Antes: .eq('status', 'delivered') → campanhas enviadas não apareciam no chat
  ...
  const fetchAll = useCallback(async () => {
    ...
    await fetchLidMap();
    await Promise.all([fetchMessageLogs(userId), fetchCampaignSends(userId), fetchSavedContacts()]);
    setLoading(false);
  ...
  const fetchProfilePicture = useCallback(
    async (phone: string, force = false, instanceId?: string | null): Promise<string | null> => {
      try {
  ...
  const autoResolveGroupMetadata = useCallback(
    async (conversationsToCheck: Conversation[]) => {
      const token = await getToken();
      const userId = await getUserId();
      if (!token || !userId) return;

      const unresolvedGroups = conversationsToCheck
        .filter((conversation) => {
          if (!isGroupPhone(conversation.phone)) return false;
          if (conversation.contactName && conversation.contactName !== "Grupo") return false;
          const saved =
            safeMapGet(savedContacts, conversation.phone) ||
            safeMapGet(savedContacts, normalizeConversationPhone(conversation.phone));
      ...
      for (const conversation of unresolvedGroups) {
        ...
        try {
          ...
          const existing =
            safeMapGet(savedContacts, zapiPhone) || safeMapGet(savedContacts, normalizeConversationPhone(zapiPhone));
          await savedContactsApi.upsert(token, {
            phone: zapiPhone,
            name: resolvedName || existing?.name || "",
            user_id: userId,
      ...
      if (unresolvedGroups.length > 0) await fetchSavedContacts();
    },
  ...
  useEffect(() => {
    ...
    const setupRealtime = async () => {
      ...
      const ch1 = supabase
        .channel(`msg-logs-rt-${userId}-${Date.now()}`)
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "message_logs", filter: `user_id=eq.${userId}` },
          (payload) => {
            if (payload.eventType === "INSERT" || payload.eventType === "UPDATE") {
              ...
              const hasContent = Boolean(
                record.message_received || (record.response_sent && record.response_sent !== "__processing__"),
              );
              ...
              setMessageLogs((prev) => {
                if (
                  ...
                  !record.keyword_matched?.startsWith("__")
                ) {
                  try {
                ...
                const next = exists ? prev.map((m) => (m.id === record.id ? record : m)) : [...prev, record];
      ...
      const ch2 = supabase
        .channel(`camp-sends-rt-${userId}-${Date.now()}`)
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "campaign_sends", filter: `user_id=eq.${userId}` },
          (payload) => {
            ...
            if (payload.eventType === "INSERT" || payload.eventType === "UPDATE") {
              // FIX: não remove do estado quando status é 'pending' ou 'failed'
              // Antes removia, causando flicker na UI quando o webhook atualizava
              // Apenas esconde via isCampaignMessageVisible no useMemo
              const shouldKeep = ["delivered", "sent", "read", "pending", "failed"].includes(record.status || "");
              setCampaignSends((prev) => {
  ...
  const conversations: Conversation[] = useMemo(() => {
    ...
    const filteredLogs = (() => {
      ...
      if (hasKnownInstanceFilter) {
        const knownIdSet = new Set([...(knownInstanceIds || []), ...(knownInstanceNames || [])]);
    ...
    filteredLogs.forEach((log) => {
      ...
      if (inboundContent || (isInternal && log.message_received)) {
        ...
        const externalMessageId =
          parseExternalMessageIdFromContent(parsed.rest).externalMessageId ||
          String(log.keyword_matched || "").match(/^__msg_import__:(.+)$/)?.[1] ||
          null;

        // FIX: usa colunas nativas sender_name/sender_phone/sender_photo quando disponíveis
        // Antes dependia só do prefix [sender:...] que pode não estar presente em mensagens antigas
      ...
      if (log.response_sent && log.response_sent !== "__processing__") {
        ...
        if (isSummary) {
          const hasDetailedFlowAround = messageLogs.some((candidate) => {
            ...
            const timeDiff = Math.abs(
              toMillis(candidate.timestamp || candidate.created_at) - toMillis(log.timestamp || log.created_at),
            );
        ...
        if (isFlowSend) displayKeyword = log.keyword_matched?.replace("__flow_send__:", "") || null;
        else if (isManual) displayKeyword = null;

        const sentMessage = parseExternalMessageIdFromContent(log.response_sent);
        const externalMessageId =
          sentMessage.externalMessageId ||
          String(log.keyword_matched || "").match(/^__msg_import__:(.+)$/)?.[1] ||
          null;
    ...
    const filteredCampaignSends = (() => {
      if (filterInstanceName) return campaignSends.filter((s) => s.instance_name === filterInstanceName);
      ...
      .map(([phone, msgs]) => {
        ...
        const preferredInstanceId =
          filterInstanceId && filterInstanceId !== "all"
            ? filterInstanceId
            : latestInboundLog?.instance_id ||
              safeMapGet(groupSourceInstances, phone) ||
              safeMapGet(groupSourceInstances, normalizedPhone) ||
              null;
        ...
        const profilePictureUrl =
          localManualPhotos.get(phone) ||
          savedPhoto ||
          groupOnlyPhoto ||
          localManualPhotos.get(normalizedPhone) ||
          null;
        ...
        return {
          ...
          agent_stage:
            saved?.agent_stage ||
            JSON.parse(localStorage.getItem("temp_contact_stages") || "{}")[normalizedPhone]?.stage ||
            JSON.parse(localStorage.getItem("temp_contact_stages") || "{}")[normalizedPhone] ||
            "triage",
          deal_value:
            saved?.deal_value ||
            (JSON.parse(localStorage.getItem("temp_contact_stages") || "{}")[normalizedPhone] as any)?.deal_value ||
            0,
          closing_date:
            saved?.closing_date ||
            (JSON.parse(localStorage.getItem("temp_contact_stages") || "{}")[normalizedPhone] as any)?.closing_date ||
            null,
          priority:
            saved?.priority ||
            (JSON.parse(localStorage.getItem("temp_contact_stages") || "{}")[normalizedPhone] as any)?.priority ||
            "normal",
          description:
            saved?.description ||
            (JSON.parse(localStorage.getItem("temp_contact_stages") || "{}")[normalizedPhone] as any)?.description ||
            null,
          responsible_ids:
            saved?.responsible_ids ||
            (JSON.parse(localStorage.getItem("temp_contact_stages") || "{}")[normalizedPhone] as any)
              ?.responsible_ids ||
            null,
          deal_metadata:
            saved?.deal_metadata ||
            (JSON.parse(localStorage.getItem("temp_contact_stages") || "{}")[normalizedPhone] as any)?.deal_metadata ||
            {},
        };
      })
  ...
  const unresolvedGroupKey = conversations
    .filter(
      (c) =>
        isGroupPhone(c.phone) &&
        (!c.contactName || c.contactName === "Grupo" || c.contactName === "Comunidade" || c.contactName === "Canal"),
    )
    .map((c) => c.phone)
    .sort()
    .join("|");
  ...
  const sendMessage = useCallback(
    ...
    async (phone: string, message: string, options: SendMessageOptions = {}) => {
      ...
      if (options.preferredInstanceId) body.instanceId = options.preferredInstanceId;
      else if (filterInstanceId && filterInstanceId !== "all") body.instanceId = filterInstanceId;
      ...
      if (error) {
        ...
        if (response instanceof Response) {
          try {
            ...
            throw new Error(
              errorData?.message || errorData?.error || `Falha ao enviar mensagem (status ${response.status})`,
            );
          } catch {
  ...
  const deleteConversation = useCallback(async (phone: string) => {
    ...
    setMessageLogs((prev) => prev.filter((m) => m.phone !== phone && m.phone !== zapiPhone));
    setCampaignSends((prev) => prev.filter((s) => s.phone !== phone && s.phone !== zapiPhone));
  ...
  return {
    ...
    updateContactStage: async (phone: string, stage: string, additionalData?: Partial<SavedContact>) => {
      ...
      try {
        ...
        console.warn("Failed to save stage to database:", e);
