...
import { Calendar, Clock, Users, Upload, UserPlus, Eye, Video, Workflow, Tag, Filter, Image } from "lucide-react";
...
// ─── Safe image with React-rendered fallback (no innerHTML / XSS risk) ───────

function SafeCardImage({ src, alt, badge }: { src: string; alt: string; badge: string }) {
  ...
  if (failed || !src.trim()) {
    return (
      <div className="relative w-full h-40 bg-muted flex flex-col items-center justify-center text-muted-foreground">
        ...
        <span className="text-xs">{src.trim() ? "Imagem não disponível" : "Sem imagem"}</span>
        <span className="absolute top-2 right-2 text-[10px] px-2 py-0.5 bg-background/80 backdrop-blur rounded-full font-medium">
          {badge}
        </span>
      </div>
    );
...
export function CreateCampaignDialog({ open, onOpenChange }: CreateCampaignDialogProps) {
  ...
  useEffect(() => {
    ...
    const loadTagsData = async () => {
      try {
        const [{ data: tagsRes }, { data: colorsRes }] = await Promise.all([
          supabase.functions.invoke("zapi-chat-actions", { body: { action: "list-tags" } }),
          supabase.functions.invoke("zapi-chat-actions", { body: { action: "tag-colors" } }),
        ]);
  ...
  const dedup = (list: Array<{ phone: string; name: string }>) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.name || (!isFlow && !formData.template_id) || (isFlow && !formData.flow_id)) {
      toast({
        title: "Erro",
        description: isFlow
          ? "Nome da campanha e fluxo são obrigatórios"
          : "Nome da campanha e modelo são obrigatórios",
        variant: "destructive",
      });
      return;
    }
    if (formData.schedule_type === "scheduled" && !formData.scheduled_at) {
      toast({ title: "Erro", description: "Selecione uma data e hora para agendamento", variant: "destructive" });
      return;
    }

    let targetContacts: Array<{ phone: string; name: string }> = [];
    ...
    if (formData.contact_selection === "all") {
      ...
      targetContacts = formData.specific_contacts
        ...
        .filter(Boolean)
        .map((raw) => ({
          phone: /@lid$/i.test(raw) ? raw.toLowerCase() : raw.replace(/\D/g, ""),
          name: "Cliente",
        }))
        .filter((c) => c.phone);
    ...
    if (formData.remove_duplicates) targetContacts = dedup(targetContacts);

    if (targetContacts.length === 0) {
      toast({ title: "Erro", description: "Adicione pelo menos um contato à campanha", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Erro", description: "Erro ao criar campanha. Tente novamente.", variant: "destructive" });
    }
  };
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const parsePhone = (raw: string) => {
      ...
      return /@lid$/i.test(s) ? s.toLowerCase() : s.replace(/\D/g, "");
    };
      ...
      Papa.parse(file, {
        ...
        complete: (results) => {
          const parsed = (results.data as any[])
            .map((row) => ({
              phone: parsePhone(
                row.telefone ?? row.phone ?? row.numero ?? row.Telefone ?? row.Phone ?? row.Numero ?? "",
              ),
              name: row.nome ?? row.name ?? row.Name ?? row.Nome ?? "Cliente",
            }))
            .filter((c) => c.phone);
          setImportedContacts(parsed);
          toast({ title: "Sucesso", description: `${parsed.length} contatos importados` });
        },
        error: () => toast({ title: "Erro", description: "Erro ao ler o arquivo CSV", variant: "destructive" }),
      });
      ...
      file.arrayBuffer().then((buf) => {
        try {
          ...
          if (!parsed.length) {
            ...
            parsed = matrix
              .map((row) => ({
                phone: parsePhone(String(row?.[0] ?? "")),
                name: String(row?.[1] ?? "Cliente"),
              }))
          ...
          toast({ title: "Erro", description: "Não foi possível ler a planilha", variant: "destructive" });
        }
      });
    } else {
      toast({
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Configure sua campanha de mensagens personalizadas</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v: "template" | "flow") =>
                  setFormData((p) => ({ ...p, content_type: v, template_id: "", flow_id: "" }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            {formData.content_type === "template" && (
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setFormData((p) => ({ ...p, template_id: v }))}
                >
                  <SelectTrigger>
            ...
            {formData.content_type === "flow" && (
              <div>
                ...
                <Select value={formData.flow_id} onValueChange={(v) => setFormData((p) => ({ ...p, flow_id: v }))}>
                  <SelectTrigger>
          ...
          {/* Carousel preview */}
          {selectedTemplate?.type === "carrossel" && selectedTemplate.carouselCards && (
            <div className="space-y-4">
              <div className="flex items-center gap-2">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label>Adicionar etiqueta na campanha</Label>
              <Select value={formData.tag_id} onValueChange={(v) => setFormData((p) => ({ ...p, tag_id: v }))}>
                <SelectTrigger>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setFormData((p) => ({ ...p, schedule_type: v }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setFormData((p) => ({ ...p, delay_seconds: parseInt(e.target.value) || 2 }))}
              />
              <p className="text-xs text-muted-foreground mt-1">Recomendado: 2–5 segundos.</p>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex-1">
                ...
                <Select
                  ...
                  onValueChange={(v) => {
                    setFormData((p) => ({ ...p, contact_selection: v }));
                    if (v !== "import") setImportedContacts([]);
                  }}
                >
                  <SelectTrigger>
                    <SelectValue />
                ...
                <div className="flex items-center justify-between p-3 bg-accent/30 rounded-lg border border-border/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <Label className="text-sm font-semibold cursor-pointer" htmlFor="remove_duplicates">
                        Remover Duplicados
                      </Label>
                      <span className="text-[10px] text-muted-foreground">
                        Evita envios repetidos para o mesmo número
                      </span>
                    </div>
                  </div>
                  <Switch
                    ...
                    onCheckedChange={(v) => setFormData((p) => ({ ...p, remove_duplicates: v }))}
                  />
                </div>
              )}
            </div>
            ...
            {formData.contact_selection === "manual" && (
              <div>
                ...
                <Textarea
                  ...
                  rows={5}
                  onChange={(e) => setFormData((p) => ({ ...p, specific_contacts: e.target.value }))}
                  placeholder={"Digite os números (um por linha)\n5511999999999\n123456789@lid"}
                />
                <p className="text-xs text-muted-foreground mt-1">
                  Um número por linha. Aceita formato normal (5511999999999) ou identificador @lid.
                </p>
              </div>
            )}
            ...
            {formData.contact_selection === "import" && (
              <div className="space-y-3">
                ...
                {importedContacts.length > 0 && (
                  <div className="bg-muted/50 p-3 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium">{importedContacts.length} contatos importados</span>
                      <Button type="button" variant="ghost" size="sm" onClick={() => setImportedContacts([])}>
                        Limpar
                      </Button>
                    </div>
                ...
                <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
                  ...
                  <pre className="text-xs text-blue-700 dark:text-blue-300 font-mono">{`telefone,nome\n5511999999999,João Silva\n5511888888888,Maria Santos`}</pre>
                </div>
              </div>
            )}
          </div>
