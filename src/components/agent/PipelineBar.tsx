          ...
          <div className="space-y-1">
              ...
              {pipelines.map((p) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "w-full flex items-center justify-between gap-2 px-2 py-2 rounded-md text-left hover:bg-muted transition-colors",
                      isActive && "bg-primary/10"
                    )}
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <DollarSign className={cn("w-3.5 h-3.5 shrink-0", isActive ? "text-primary" : "text-muted-foreground")} />
                      <div className="min-w-0">
                        <div className="text-xs font-medium truncate">{p.name}</div>
                        <div className="text-[10px] text-muted-foreground truncate">
                          {(p.department || "—")} · {p.stages.filter(s => s.id !== 'all').length} etapas
                        </div>
                      </div>
                    </div>
            ...
            <div className="border-t border-border pt-1 mt-1 flex gap-1">
              <Button
                ...
                onClick={() => { setIsPickerOpen(false); openCreateDialog(); }}
              >
              ...
              <Button
                ...
                variant="ghost"
                className="flex-1 h-8 justify-start gap-2 text-xs"
                onClick={() => { setIsPickerOpen(false); setIsManageOpen(true); }}
              >
                <LayoutGrid className="w-3.5 h-3.5" /> Gerenciar
              </Button>
            </div>
          </div>
        </PopoverContent>
