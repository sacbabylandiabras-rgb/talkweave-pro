                  ...
                  <>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <Label className="text-sm font-semibold">Cards do Carrossel</Label>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => {
                            const cards = selectedNode.data.carouselCards || [];
                            if (cards.length >= 10) {
                            ...
                            setSelectedNode({
                              ...
                              data: {
                                ...selectedNode.data,
                                carouselCards: [...cards, { title: "", description: "", image: "", buttonLabel: "", buttonUrl: "" }],
                              },
                            });
                          }}
                        >
                      ...
                      {(!selectedNode.data.carouselCards || selectedNode.data.carouselCards.length === 0) && (
                        <p className="text-xs text-muted-foreground">Adicione ao menos 2 cards para o carrossel.</p>
                      )}

                      {(selectedNode.data.carouselCards || []).map((card: any, idx: number) => (
                        <Card key={idx} className="p-3 space-y-2">
                          <div className="flex items-center justify-between">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                setSelectedNode({
                                  ...selectedNode,
                                  data: { ...selectedNode.data, carouselCards: cards },
                                });
                              }}
                            >
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              setSelectedNode({ ...selectedNode, data: { ...selectedNode.data, carouselCards: cards } });
                            }}
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              placeholder="URL da imagem (1125x600 recomendado)"
                            />
                            <div className="mt-1">
                              <label className="cursor-pointer">
                                <div className="flex items-center gap-2 p-2 border rounded text-xs text-muted-foreground hover:border-primary transition-colors">
                                  <Upload className="h-3 w-3" />
                                  {card.image ? "✓ Imagem carregada" : "Upload imagem"}
                                </div>
                                <Input
                                  type="file"
                                  className="hidden"
                                  ...
                                  onChange={async (e) => {
                                    ...
                                    try {
                                      ...
                                      const fileName = `carousel_${Date.now()}_${idx}.${ext}`;
                                      const { data: uploadData, error: uploadError } = await supabase.storage
                                        .from('flow-media')
                                        .upload(fileName, file);
                                      if (uploadError) throw uploadError;
                                      const { data: urlData } = supabase.storage.from('flow-media').getPublicUrl(fileName);
                                      const cards = [...(selectedNode.data.carouselCards || [])];
                                      cards[idx] = { ...cards[idx], image: urlData.publicUrl };
                                      setSelectedNode({ ...selectedNode, data: { ...selectedNode.data, carouselCards: cards } });
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              const cards = [...(selectedNode.data.carouselCards || [])];
                              cards[idx] = { ...cards[idx], buttonLabel: e.target.value };
                              setSelectedNode({ ...selectedNode, data: { ...selectedNode.data, carouselCards: cards } });
                            }}
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              const cards = [...(selectedNode.data.carouselCards || [])];
                              cards[idx] = { ...cards[idx], buttonUrl: e.target.value };
                              setSelectedNode({ ...selectedNode, data: { ...selectedNode.data, carouselCards: cards } });
                            }}
