...
export default function TelegramGruposCanaisFluxos({
  botId, chatId, groupTitle,
}: { botId: string; chatId: number | null; groupTitle: string }) {
  ...
  async function load() {
    if (!botId || !chatId) { setFlows([]); setRuns([]); return; }
    setLoading(true);
    const [{ data: f }, { data: r }] = await Promise.all([
      supabase.from("telegram_group_flows" as any)
        .select("*").eq("bot_id", botId).eq("chat_id", chatId)
  ...
  async function runNow(flow: Flow) {
    ...
    const { data, error } = await supabase.functions.invoke("telegram-group-flow-trigger", {
      body: { flow_id: flow.id, chat_id: chatId, trigger_source: "manual" },
    });
  ...
  function openNew() {
    navigate(`/telegram/canal-free/fluxos/novo?botId=${botId}&chatId=${chatId}`);
  }

  function openEdit(flow: Flow) {
    navigate(`/telegram/canal-free/fluxos/${flow.id}?botId=${botId}&chatId=${chatId}`);
  }

  const canCreate = !!botId && !!chatId;

  return (
    <div className="space-y-5">
      {!canCreate && (
        <Card className="p-4 border-l-4 border-l-amber-500 bg-amber-500/5 text-sm text-foreground">
          Selecione um bot e um grupo/canal para gerenciar os fluxos.
        </Card>
      )}
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-semibold text-foreground flex items-center gap-2">
            <Workflow className="w-4 h-4" /> Fluxos {groupTitle ? `· ${groupTitle}` : ""}
          </h3>
          <p className="text-xs text-muted-foreground">
            Sequências automáticas de mensagens enviadas dentro deste grupo/canal.
          </p>
        </div>
      ...
      <Tabs defaultValue="list">
        ...
        <TabsContent value="list" className="space-y-2 pt-3">
          ...
          {!loading && canCreate && flows.length === 0 && (
            <Card className="p-6 text-center text-sm text-muted-foreground">
              Nenhum fluxo criado ainda para este grupo/canal.
            </Card>
          )}
          {flows.map((f) => (
