...
const EnviarMensagem = () => {
  ...
  const sendResolvedContent = async (phone: string, nome?: string, preConvertedMedia?: { base64: string, name: string, type: string } | null) => {
    ...
    if (activeTab === 'botoes' && botoesAcao.some(b => b.label)) {
      ...
      await sendButtonActions(phone, mensagemPersonalizada, validButtons.map(btn => ({ id: btn.id, type: btn.type, label: btn.label, ...(btn.type === 'CALL' && { phone: btn.phone }), ...(btn.type === 'URL' && { url: btn.url }), ...(btn.type === 'COPY' && { copyText: btn.copyText }) })), titulo || undefined, rodape || undefined);
      return mensagemPersonalizada;
    }
    if (activeTab === 'lista' && opcoes.some(o => o.title)) {
      const validOptions = opcoes.filter(opt => opt.title.trim() !== '');
      await sendOptionList(phone, mensagemPersonalizada, { title: tituloLista || 'Opções', buttonLabel: labelBotaoLista || 'Ver opções', options: validOptions });
      return mensagemPersonalizada;
    }
