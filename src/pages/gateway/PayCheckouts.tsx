                    ...
                    <TableRow key={ck.id} className="border-[#2A2A2A]">
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          ...
                          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => { const publishedBase = "https://pay.zaplynx.com"; const defaultLink = `${publishedBase}/checkout/${ck.slug || ck.id}`; if (savedDomain) { const customLink = `https://${savedDomain}/${ck.slug || ck.id}`; navigator.clipboard.writeText(`${customLink}\n${defaultLink}`); toast.success("Links copiados! (personalizado + padrão)"); } else { navigator.clipboard.writeText(defaultLink); toast.success("Link copiado!"); } }}><Copy className="w-3.5 h-3.5" /></Button>
